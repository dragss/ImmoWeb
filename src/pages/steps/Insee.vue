<template>
  <div class="flex justify-center items-center flex-col mt-6">
    ...
    <div class="mt-10" v-if="!loading && graph">
...
export default {
  ...
  created() {
    if (this.address && this.address.code_commune) {
      this.codeCommune = this.address.code_commune;
      if (!this.insee) {
        this.loadComune();
      } else {
        this.graph = this.insee;
      }
    }
  },
  computed: {
    ...mapGetters(['address', 'insee']),
  },
  methods: {
    ...mapActions(['setInsee']),
    async loadComune() {
      ...
      if (res1.data && res1.data.Cellule) {
        ...
        this.$set(this.graph, 'logmentType', {
          nbResidentPct,
          nbSecPct,
          nbVacPct,
        });
      ...
      if (res2.data && res2.data.Cellule) {
        ...
        this.$set(this.graph, 'sociopro', pourcentage);
      }
      ...
      if (res3.data && res3.data.Cellule) {
        ...
        this.$set(this.graph, 'proprio', pourcentage3);
      }
    ...
    handleSave() {
      ...
      setTimeout(() => {
        this.clicked = false;
        this.$router.push('/pdf-gen/step/3');
      }, 1000);
    },
  },
  data() {
